...
final private[users] class Entities[F[_]](users: Users[F])(implicit S: Effect[F], EH: ErrorHandler[F, UserError])
  ...
  private def service: HttpRoutes[F] = HttpRoutes.of[F] {
    case GET → (Root / "user" / LongVar(byId)) ⇒
      S.flatMap(users.findById(byId)) {
        case Some(user) ⇒ Ok(user.asJson)
        case None       ⇒ NotFound(Message(Error, s"User [$byId] not found").asJson)
      }

    case GET → (Root / "users") ⇒
      S.flatMap(users.findAll) { users ⇒
        Ok(users.asJson)
      }

    case req @ POST → (Root / "user") ⇒
      req.as[UserDefinition].flatMap { user ⇒
        users.create(user) *> Created(user.asJson)
      }

    case GET → (Root / "index") ⇒ Ok(Message(Info, "Welcome to user service").asJson)
  }
